...
Beacon::Beacon()
    : mX(0.), mY(0.), mLocationId(0)
{
}

//copy constructor
Beacon::Beacon(const Beacon &beacon)
    : mX(beacon.mX), mY(beacon.mY), mId(beacon.mId), mName(beacon.mName), mLocationName(beacon.mLocationName), mLocationId(beacon.mLocationId)
{
}
...
IBeacon::IBeacon()
    : Beacon(), majorr(0), minorr(0)
{
}

//copy constructor
IBeacon::IBeacon(const IBeacon &iBeacon)
    : Beacon(iBeacon), uuid(iBeacon.uuid), majorr(iBeacon.majorr), minorr(iBeacon.minorr)
{
}
...
int IBeacon::getmajorr() const
{
  return majorr;
}

int IBeacon::getminorr() const
{
  return minorr;
}

void IBeacon::setmajorr(const unsigned int majorr)
{
  IBeacon::majorr = majorr;
}

void IBeacon::setminorr(const unsigned int minorr)
{
  IBeacon::minorr = minorr;
}
...
BeaconMeas::BeaconMeas()
    : mBeaconPtr(0), mRssi(TRANSMITTER_POINT_UNUSED)
{
}
...
BeaconMeas::BeaconMeas(const BeaconMeas &beaconMeas)
    : mBeaconId(beaconMeas.mBeaconId), mBeaconPtr(beaconMeas.mBeaconPtr), mRssi(beaconMeas.mRssi), mDistance(beaconMeas.mDistance)
{
}
