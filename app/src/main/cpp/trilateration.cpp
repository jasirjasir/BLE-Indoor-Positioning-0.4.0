...
Trilateration::Trilateration(const Trilateration &trilat)
    : mLocationBeacons(trilat.mLocationBeacons), mBeaconMeas(trilat.mBeaconMeas), mXY(trilat.mXY), mCurLocationId(trilat.mCurLocationId), mErrorMessage(trilat.mErrorMessage)
{
}
                                      ...
                                      vector<double> b)
{
  ...
  mXY[1] = xy[1];
  printf( " x = %lf y = %lf \n", mXY[0], mXY[1] );
  //__android_log_print(ANDROID_LOG_VERBOSE, "beacon", " --x = %s --y= %s", to_string( mXY[0]).c_str(), to_string( mXY[1]).c_str());
